...
X = pandas.read_csv('../doctor/train_X.csv')
#读取训练数据 实际结果部分(上述112位顾客真实的怀孕状态，0表示女娃，1表示男孩，2表示没有怀孕)
y = pandas.read_csv('../doctor/train_y.csv')
#y = pandas.read_csv('train_y.csv').values.ravel()  # 注意：sklearn要求标签是一维数组，需用ravel()转成一维

#创建一个机器人
doctor = KNN(k=10)
#doctor2 = KNeighborsClassifier(n_neighbors=5)
#训练机器人
doctor.fit(X, y)

#下面使用38位顾客的数据测试机器人诊断效果
#读取38位顾客的气色,气味,脉相,体温等特征数据
test_X = pandas.read_csv('../doctor/test_X.csv')
#诊断结果存放到result数组中
result = doctor.predict(test_X)

#打印输出诊断结果，与实际的结果比较
#读取38位顾客怀孕状态的实际值(0表示女娃，1表示男孩，2表示没有怀孕)
test_y = pandas.read_csv('../doctor/test_y.csv')
labels = ['女娃','男孩','没有怀孕']
i = 0 # 样本总数
predictOKNum = 0 # 正确的诊断数
print("编号,诊断值,实际值,")
